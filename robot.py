...
def player_maker(player_list):
    ...
    while user_choose == "yes" or user_choose =='y':
        user_input = input('Input name Player ')
        if len(user_input) <= 3:
            continue
        player_list.append(str.capitalize(user_input))
        user_choose = input('More plaers - y/n')
...
def bot_maker(start_sum, player_list):
    ...
    while wallet >= min(details_damage_price.values()):
        ...
        print(f'{string_1:=^80}')
        print(f'{"You can bay":-^80}')
        print('{detail:^^80}'.format(detail="Damage deal details"))
        for gun, price in details_survive_price.items():
            print(f"{gun} - {price}")
        print('{detail:^^80}'.format(detail="Survive details"))
        for survive_datail, price in details_survive_price.items(): #[(1, 2), (3, 4)]
            print(f"{survive_datail} - {price}")
